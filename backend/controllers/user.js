...
export const signUp = async (req, res, next) => {
  try {
    ...
    const { channel, channel_id, email, username, user_id } =
    ...
    if (nickname === undefined || nickname === null || nickname === "") {
      res
        .status(400)
        .json({ success: false, message: "nickname is required.", data: {} });
    }
    //註冊使用者到資料庫
...
export const getUserProfile = async (req, res, next) => {
  ...
  const current_time = query.time_choosen
    ? query.time_choosen
    : new Date().toISOString().split("Z")[0].split("T").join(" ");
  ...
  await User.get_user(user_id, current_time)
    .then(async (results) => {
      console.log(results);
      if (results.length > 0) user_status = true;
      ...
      var Data = {
        ...
        data: {
          ...
          nickname: results[0].nickname.slice(
            1,
            results[0].nickname.length - 1
          ),
      ...
      if (results[0].barcode)
        Data.data["barcode"] = results[0].barcode.slice(
          1,
          results[0].barcode.length - 1
        );
      ...
      for (let i = 0; i < results.length; ) {
        ...
        var wallet_obj = {
          ...
          wallet_name: results[i].wallet_name.slice(
            1,
            results[i].wallet_name.length - 1
          ),
          wallet_total: results[i].wallet_total,
          wallet_description: results[i].wallet_description.slice(
            1,
            results[i].wallet_description.length - 1
          ),
        ...
        if (results[i].wallet_barcode)
          wallet_obj["wallet_barcode"] = results[i].wallet_barcode.slice(
            1,
            results[i].wallet_barcode.length - 1
          );
        ...
        if (results[i].selected == 1) {
            ...
            .then((results) => {
              ...
              for (var i = 0; i < results.length; ++i) {
                var tag_obj = {
                  ...
                  tag_name: results[i].tag_name.slice(
                    1,
                    results[i].tag_name.length - 1
                  ),
          ...
          if (results[i].record_num) {
            for (j = 0; j < results[i].record_num; ++j) {
              // 若已經沒有資料或已是不同wallet,則提早break
              if (
                i + j >= results.length ||
                results[i + j].wallet_id != results[i].wallet_id
              )
                break;
              var record_obj = {
                ...
                record_name: results[i + j].record_name.slice(
                  1,
                  results[i + j].record_name.length - 1
                ),
                record_description: results[i + j].record_description.slice(
                  1,
                  results[i + j].record_description.length - 1
                ),
                  ...
                  if (result.length > 0) {
                    for (var i = 0; i < result.length; ++i) {
                      var record_debtor_obj = {
                        ...
                        debtor_name: result[i].debtor_name.slice(
                          1,
                          result[i].debtor_name.length - 1
                        ),
        ...
        .then((results) => {
          for (var i = 0; i < results.length; ++i) {
            var debtor_obj = {
              ...
              debtor_name: results[i].debtor_name.slice(
                1,
                results[i].debtor_name.length - 1
              ),
