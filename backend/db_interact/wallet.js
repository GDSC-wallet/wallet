...
const search_record = async (wallet_id, search_str) => {
    return new Promise( async (resolve, reject) => {
        // 搜尋record
        var sql = `START TRANSACTION; SELECT * FROM wallet_record WHERE record_wallet_id = ? AND record_name REGEXP ?; COMMIT`;
        pool.getConnection( async (err, conn) => {
            if(err) {
                print_error(err);
                reject(err);
            } else {
                await conn.query(sql, [wallet_id, conn.escape(search_str)], (err, results, fields) => {
                    if(err) {
                        print_error(err);
                        reject(err);
                    } else {
